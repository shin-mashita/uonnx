...
void arena_add_initializer(TensorProto * initializer, TensorArena * arena)
{
    ...
    t = tensor_alloc_nodatas(initializer->name, initializer->data_type, initializer->dims, initializer->n_dims, 1);
...
void arena_add_intermediate(PlanProto * plan, TensorArena * arena)
{
    ...
    t = tensor_alloc_nodatas(plan->name, plan->type, plan->dims, plan->n_dims, 0);
