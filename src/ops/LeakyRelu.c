...
static int LeakyRelu_init(Node * n)
{
...
static int LeakyRelu_exit(Node * n)
{
...
static int LeakyRelu_reshape(Node * n)
{
	Tensor * x = n->inputs[0];
...
static void LeakyRelu_float16(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * x = n->inputs[0];
	Tensor * y = n->outputs[0];
...
static void LeakyRelu_float32(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * x = n->inputs[0];
	Tensor * y = n->outputs[0];
...
static void LeakyRelu_float64(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * x = n->inputs[0];
	Tensor * y = n->outputs[0];
