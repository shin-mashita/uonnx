...
static int Gemm_reshape(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	...
	if(pdat->m <= 0 || pdat->n <= 0 || pdat->k <= 0)
		return 0;
	if((n->ninputs > 2) && !onnx_tensor_broadcast_is_valid(n->inputs[2], (int[]){ pdat->m, pdat->n }, 2))
		return 0;
	return onnx_tensor_reshape(y, (int[]){ pdat->m, pdat->n }, 2, a->type);
}

static void Gemm_int32(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * y = n->outputs[0];
	Tensor * a = n->inputs[0];
	Tensor * b = n->inputs[1];
	Tensor * c = (n->ninputs > 2) ? n->inputs[2] : NULL;
...
static void Gemm_uint32(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * y = n->outputs[0];
	Tensor * a = n->inputs[0];
...
static void Gemm_uint64(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * y = n->outputs[0];
	Tensor * a = n->inputs[0];
...
static void Gemm_bfloat16(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * y = n->outputs[0];
	Tensor * a = n->inputs[0];
...
static void Gemm_float16(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * y = n->outputs[0];
	Tensor * a = n->inputs[0];
...
static void Gemm_float32(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * y = n->outputs[0];
	Tensor * a = n->inputs[0];
...
static void Gemm_float64(Node * n)
{
	struct operator_pdata_t * pdat = (struct operator_pdata_t *)n->priv;
	Tensor * y = n->outputs[0];
	Tensor * a = n->inputs[0];
