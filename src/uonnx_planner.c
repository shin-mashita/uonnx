...
{
    PlannerProto * planner = planner__planner__unpack(NULL, len, (uint8_t *)buf);
    return planner;
}
    ...
    if(fp)
    {
		...
		if(l > 0)
        {
            ...
            if(buf)
            {
                ...
                planner = planner__planner__unpack(NULL, len, (uint8_t *)buf);
                free(buf);
            }
        }
        fclose(fp);
...
//     char ** seq = (char **)malloc(sizeof(char*)*n_plans);
//     int nseq = 0;

//     MemBlk * memblks = (MemBlk *)malloc(sizeof(MemBlk)*n_plans);
